...
export default function ProntuarioForm({ pacienteId, onSalvo }: ProntuarioFormProps) {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess("Prontuario registrado!", "O atendimento foi registrado com sucesso.");
      setAnotacoes("");
      setData(hoje);
      setTocado(false);
      setTimeout(onSalvo, 800);
    } catch {
      showError("Erro ao registrar", "Nao foi possivel salvar o prontuario. Tente novamente.");
      ...
      <form onSubmit={handleSalvar} className="bg-[#07111f] rounded-3xl p-6 space-y-5 relative overflow-hidden shadow-[0_24px_70px_rgba(15,23,42,0.18)]">
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Anotacoes Clinicas *</label>
              <textarea value={anotacoes} onChange={e => { setTocado(true); setAnotacoes(e.target.value); }} required rows={5}
                placeholder="Descreva a evolucao clinica, sintomas, diagnostico e conduta terapeutica..."
              ...
              {anotacoesInvalida && <p className="mt-1.5 text-xs text-red-400 font-medium">As anotacoes clinicas sao obrigatorias.</p>}
            </div>
          </div>
          <button type="submit" disabled={!podeEnviar}
            ...
            {salvando
              ...
              : <><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>Registrar Prontuario</>
            }
          </button>
        </div>
      </form>
