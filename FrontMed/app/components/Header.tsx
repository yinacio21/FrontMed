...
function getPageTitle(pathname: string): string {
  ...
  if (pathname.includes("/medicos") && pathname.includes("/editar")) return "Editar Médico";
  if (pathname.includes("/medicos/novo")) return "Novo Médico";
  ...
  return (
    <header className="h-20 bg-white/82 border-b border-slate-200/80 flex items-center justify-between px-6 sticky top-0 z-30 backdrop-blur-xl shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
      <div>
        ...
        <p className="text-xs font-medium text-slate-400">MediSys — Gestão Clínica</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden md:block">
            <p className="text-sm font-semibold text-slate-800 leading-none">{usuario?.nome || "Médico"}</p>
